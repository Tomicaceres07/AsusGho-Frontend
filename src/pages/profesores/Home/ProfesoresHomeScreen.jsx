...
export const ProfesoresHomeScreen = () => {
  ...
  const getActivities = async () => {
    await axios
      .post("/api/message_week/read", { type: user.type })
      .then((data) => {
  ...
  return (
    <div>
      <section className="teacher__home__home">
        <h1 className="teacher__home__title">
          <span className="teacher__home__title-p1">Liceo Militar</span>
          <span className="teacher__home__title-p2">General Paz</span>
        </h1>
        <h2 className="teacher__home__subtitle">
                ...
                <div key={index}>
                  ...
                  {item.messages && item.messages.length !== 0 ? (
                    item.messages.map((item, index) => (
                      <p key={index} className="teacher__home__board-task">
                        {item.text}
                      </p>
                    ))
                  ) : (
      ...
      <section className="teacher__home__menu">
        ...
        {user.p_type === 1 && (
          <div className="text-center mt-5">
            ...
            {isFilePicked && (
              <div>
                <button className="btn btn-success mb-3" onClick={addMenu}>
                  Agregar
                </button>
              </div>
            )}
