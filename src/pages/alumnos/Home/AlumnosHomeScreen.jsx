...
export const AlumnosHomeScreen = () => {
  ...
  const getActivities = async () => {
    await axios
      .post("/api/message_week/read", { type: true })
      .then(({ data }) => {
  ...
  return (
    <div>
      <section className="student__home__home">
        <h1 className="student__home__title">
          <span className="student__home__title-p1">Liceo Militar</span>
          <span className="student__home__title-p2">General Paz</span>
        </h1>
        <h2 className="student__home__subtitle">
                ...
                <div key={activityIndex}>
                  ...
                  {item.messages && item.messages.length !== 0 ? (
                    item.messages.map((item, messageIndex) => (
                      <p
                        key={messageIndex}
                        className="student__home__board-task"
                      >
                        {item.text}
                      </p>
                    ))
